...
The `wasm-bindgen` layer, ~17k lines across 14 modules, one per crate domain. Every
...
| Path                               | Feeds                                  | Contents                                                                                                                                                                                                |
| ---------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mod.rs`                           | —                                      | `pub mod` for each of the 14 domains. **No glob re-exports**                                                                                                                                            |
...
| `core/market_data.rs`              | `core`                                 | `DiscountCurve`, `HazardCurve`, `ForwardCurve`, `VolCube`, `FxDeltaVolSurface`, `FxMatrix`, `FxConversionPolicy`, `FxRateResult`, `FxQuoteConvention`, `FxPairConvention`, `fxMarketPair`, `fxPairConvention`, `fxPipSize`, `invertFxRate`, `nelsonSiegelYields` |
| `core/math.rs`                     | `core`                                 | Cholesky, statistics, special functions, compensated summation, `countConsecutive`                                                                                                                      |
| `core/credit.rs`                   | `core`                                 | `analyzeExchangeOffer`, `analyzeLme` (liability management)                                                                                                                                             |
...
| `factor_model/mod.rs`              | `factor_model.credit`                  | `CreditFactorModel`, `CreditCalibrator`, `decomposeLevels`, `decomposePeriod`, `FactorCovarianceForecast`                                                                                               |
...
| `monte_carlo/mod.rs`               | `monte_carlo`                          | European / Asian / American / Heston pricers plus Black-Scholes helpers                                                                                                                                 |
...
| `valuations/credit.rs`             | `valuations.credit`                    | Structural-credit factories: Merton, CreditGrades, dynamic recovery, endogenous hazard, toggle exercise                                                                                                 |
| `valuations/credit_derivatives.rs` | `valuations.creditDerivatives`         | CDS-family example payload factories                                                                                                                                                                    |
| `valuations/correlation/mod.rs`    | `valuations.correlation`               | Copulas, recovery models, joint probabilities, `nearestCorrelation`, tranche loss statistics                                                                                                            |
| `valuations/analytic.rs`           | `valuations`                           | Closed forms: `bsPrice`, `bsGreeks`, `bsImpliedVol`, `black76ImpliedVol`, `barrierCall`, `asianOptionPrice`, `lookbackOptionPrice`, `quantoOptionPrice`, `vanillaExpiryPayoff`                          |
| `valuations/fourier.rs`            | `valuations`                           | COS-method pricers: `bsCosPrice`, `vgCosPrice`, `mertonJumpCosPrice`                                                                                                                                    |
...
| `valuations/sabr.rs`               | `valuations`                           | `SabrParameters`, `SabrModel`, `SabrSmile`, `SabrCalibrator`                                                                                                                                            |
