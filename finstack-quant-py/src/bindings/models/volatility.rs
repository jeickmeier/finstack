...
fn get_surface_vol(surface: &PyVolSurface, expiry: f64, strike: f64) -> PyResult<f64> {
    vol::get_surface_vol(&surface.inner, expiry, strike).map_err(display_to_py)
}
...
fn get_cube_vol(cube: &PyVolCube, expiry: f64, tenor: f64, strike: f64) -> PyResult<f64> {
    vol::get_cube_vol(&cube.inner, expiry, tenor, strike).map_err(display_to_py)
}
...
fn get_cube_normal_vol(cube: &PyVolCube, expiry: f64, tenor: f64, strike: f64) -> PyResult<f64> {
    vol::get_cube_normal_vol(&cube.inner, expiry, tenor, strike).map_err(display_to_py)
}
...
fn get_fx_delta_pillar_vols(
    ...
    vol::get_fx_delta_pillar_vols(&surface.inner, expiry_index).map_err(display_to_py)
}
...
fn get_fx_delta_vol(
    ...
    vol::get_fx_delta_vol(&surface.inner, expiry, strike, forward).map_err(display_to_py)
}
