# finstack-quant-monte-carlo benchmarks
...
The package name is `finstack-quant-monte-carlo` (hyphen) even though the directory is
`monte_carlo` — `cargo bench -p finstack-quant-monte_carlo` will not resolve.
...
cargo bench -p finstack-quant-monte-carlo --bench mc_hot_paths -- --quick
cargo bench -p finstack-quant-monte-carlo -- rough_heston_step    # filter by group name
cargo bench -p finstack-quant-monte-carlo -- --save-baseline before
cargo bench -p finstack-quant-monte-carlo -- --baseline before
```
...
directory; they run under `mise run rust-test-slow`. This crate has no `tests/`
directory — its unit tests live in-crate and run via
