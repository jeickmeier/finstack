...
//!
//! This module builds `E[IM(t)]` deterministically from the current ISDA SIMM
//! number and a decay profile ([`im_profile_from_simm`]) — the standard
//! practitioner approximation. Carrying per-path IM through the stochastic
//! exposure engine is a separate, later extension and is intentionally not
...
///
/// The base IM is `calculator.calculate_from_sensitivities(sensitivities,
...
pub fn im_profile_from_simm(
    ...
    let (base_im, _breakdown) = calculator.calculate_from_sensitivities(sensitivities, currency);
...
mod tests {
    ...
    fn im_profile_from_simm_scales_base_im_by_decay() {
        ...
        let (base_im, _) = calc.calculate_from_sensitivities(&sens, Currency::USD);
