...
| `cap_floor/` | Caps, floors, collars | `CapFloorVolType` defaults to `Auto` (lognormal quote; Black-76 when forward and strike are positive, otherwise convert to an equivalent normal vol and price with Bachelier). A normal-vol surface must set `vol_type = Normal`. `Lognormal` is Black 1976, `ShiftedLognormal` is Black on `F+shift`/`K+shift`, `Normal` is Bachelier. Compounded overnight-RFR coupons support lookback, observation shift, rate cutoff and a business-day payment delay; discounting runs to the delayed payment date | no |
| `swaption/` | European and Bermudan swaptions | Black-76 / Bachelier / SABR for Europeans; HW1F tree, LSMC, LMM-BGM and Cheyette-rough for Bermudans. Physical settlement uses the physical annuity, cash settlement the par-yield annuity. HW1F surface calibration accepts 1Y/6M/3M fixed-leg frequencies and rejects anything else rather than silently defaulting to 6M | no |
