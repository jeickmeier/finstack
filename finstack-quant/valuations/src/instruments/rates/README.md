...
| `cap_floor/` | Caps, floors, collars | `CapFloorVolType` defaults to `Auto` (Black when forward and strike are positive, else Bachelier). `Lognormal` is Black 1976, `ShiftedLognormal` is Black on `F+shift`/`K+shift`, `Normal` is Bachelier. Compounded overnight-RFR coupons support lookback, observation shift, rate cutoff and a business-day payment delay; discounting runs to the delayed payment date | no |
