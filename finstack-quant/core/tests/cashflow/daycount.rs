...
fn actact_isda_leap_year_crossing_nov_to_mar() {
    ...
    let end = d(2024, 3, 1);
    let yf = day_count.year_fraction(start, end, ctx).unwrap();
...
fn actact_isda_spanning_multiple_years() {
    ...
    let yf = day_count
        .year_fraction(d(2023, 1, 1), d(2026, 1, 1), ctx)
        .unwrap();
...
fn actact_isma_requires_frequency() {
    ...
    let result = day_count.year_fraction(d(2025, 1, 1), d(2025, 7, 1), ctx_no_frequency);
    assert!(
        result.is_err(),
...
fn actact_isma_with_quarterly_frequency() {
    ...
    // Note: ISMA calculation depends on the coupon period context
    let result = day_count.year_fraction(d(2025, 1, 1), d(2025, 4, 1), ctx);
...
fn act365f_feb_leap_year() {
    ...
    let yf = day_count
        .year_fraction(d(2024, 2, 1), d(2024, 3, 1), ctx)
        .unwrap();
...
fn act365f_feb_non_leap_year() {
    ...
    let yf = day_count
        .year_fraction(d(2025, 2, 1), d(2025, 3, 1), ctx)
        .unwrap();
...
fn zero_length_period_all_conventions() {
    ...
    for (day_count, ctx) in conventions {
        let yf = day_count.year_fraction(date, date, ctx).unwrap();
        assert!(
...
fn century_leap_year_rule() {
    ...
    // Year 2000 (leap year)
    let yf_2000 = day_count
        .year_fraction(d(2000, 1, 1), d(2001, 1, 1), ctx)
        .unwrap();
    assert_close(
        yf_2000,
        1.0,
