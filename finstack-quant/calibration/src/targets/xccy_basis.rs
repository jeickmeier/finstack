...
//! computed via the CIP no-FX-vol approximation in `finstack_quant_valuations::instruments::rates::xccy_swap::pricing_mtm`.
//! Today the `XccyBasisTarget` consumes generic `RateQuote::{Deposit, Fra, Swap}` to build
//! a foreign discount curve.

use crate::api::schema::XccyBasisParams;
...
/// Instrument notional and residual normalization divisor.
...
impl XccyBasisTarget {
    ...
    pub(crate) fn solve(
        ...
        if has_xccy {
            ...
            let xccy_build_ctx = crate::build::context::BuildCtx::new(
                schema_params.base_date,
                XCCY_RESIDUAL_NOTIONAL,
                xccy_curve_ids,
            );
...
mod xccy_quote_calibration_tests {
    ...
    fn xccy_basis_target_accepts_xccy_basis_quotes() {
        ...
        let params = crate::api::schema::XccyBasisParams {
            ...
            extrapolation: ExtrapolationPolicy::FlatZero,
            conventions: crate::config::RatesStepConventions {
