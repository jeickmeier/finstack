...
### Deprecated
- **`Estimate::with_num_skipped`** (`finstack-quant-monte-carlo`): engine loops
  now reject non-finite payoffs instead of censoring paths. Retained for serde
  backward compatibility; removable in `1.0.0`.
...
- **`CashflowBreakdown::interest_income_cash`** (`Option<Money>`, additive per
  ...
  money.
- **IMM / CDS-IMM schedule rolls:** `ScheduleParams` exposes IMM and CDS-IMM
  roll rules; semi-annual CDS roll-date helpers are documented for CDS-style
  schedules.
- **Term-loan step-up coupons** and improved revolving-credit cashflow export /
  validation.
- **Structured-credit tranche analytics in Python**, including deal-level market
  quote injection for Z-spread, senior fee tier wiring, Richard-Roll burnout /
  refi incentive, currency-stamped tranche metrics, and Monte Carlo as the
  default pricing engine.
- Dual licensing under **MIT OR Apache-2.0** (see `LICENSE`, `LICENSE-MIT`,
  `LICENSE-APACHE`).
...
- **`finstack-quant-statements` — swap net interest receipts were booked as
  ...
  above). Single-leg debt is unaffected: bonds and loans emit positive coupons
...
- **Structured credit correctness suite:** cash conservation, shared time basis
  for risk metrics, deferred interest vs PIK separation, fee-aware IC / OC /
  reserve sizing, OAS rate-path floating coupons, correlation bounds, non-finite
  metric rejection at the JSON boundary, and related waterfall / coverage fixes.
- **Analytics NaN / drawdown / FX handling:** performance metrics propagate NaN
  more consistently; drawdown and FX rate helpers were hardened for edge cases.
- **Internal CDS-tranche / SVI regression pins rebaselined** for CS01 (and the
  AAPL SVI formula self-test NPV) after small deterministic engine drift;
  textbook Hull–White Table 7 `par_spread` anchors are unchanged.
...
- PDE convection-diffusion solvers, Asian option continuous-averaging
  validation, discount-curve interpolation, volatility-index curves, and
  Gauss-Hermite integration documentation/validation were tightened for
  numerical stability.
